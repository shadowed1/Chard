...
echo "${RESET}"
echo ""
echo "${RED}- Chard Arch can take ${BOLD}20 to 240 minutes${RESET}${RED} depending on your CPU and storage speed. Requires ~11 GB of space. Supports ${BOLD}x86_64${RESET}${RED} and ${BOLD}ARM64${RESET}${RED}! ${RESET}"
...
echo "${GREEN}- Chard is currently in development. ${BOLD}Bugs will exist${RESET}${GREEN}, so please have a ${BOLD}USB backup${RESET}${GREEN} in case of serious mistakes.${RESET}"
echo
...
echo "${RESET}"
echo "${YELLOW}${BOLD}"
echo "If using crew-sudo please uninstall it and use Sudocrosh instead."
echo "${RESET}"
...
show_progress() {
    ...
    echo "${CYAN}Setup Runtime (including Chariot): $formatted_time ${RESET} $1"
}
...
if [ -f "$CHROMEOS_BASHRC" ]; then
    CHROME_MILESTONE=$(grep '^CHROMEOS_RELEASE_CHROME_MILESTONE=' /etc/lsb-release | cut -d'=' -f2)
    echo "$CHROME_MILESTONE" | sudo tee "$CHARD_ROOT/.chard_chrome" > /dev/null
    sudo ln -s $CHARD_ROOT/usr/bin/xkbcomp /usr/bin/xkbcomp 2>/dev/null
fi
