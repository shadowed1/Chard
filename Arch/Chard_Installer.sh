...
echo
echo "${RED}Chard Installs to ${CHARD_ROOT}${RESET}${YELLOW} - Install will eventually chroot into chard. ${RESET}${BOLD}${GREEN}This means / will be $CHARD_ROOT/ in reality.${RESET}"
echo
...
sudo chroot $CHARD_ROOT /bin/bash -c "
                        ...
                        for grp in wayland arc-bridge arc-keymintd arc-sensor android-everybody \
                            audio input uinput lp video bluetooth-audio cras usb traced-producer \
                            traced-consumer chronos-access brltty arcvm-boot-notification-server \
                            arc-mojo-proxy arc-host-clock midis suzy-q ml-core fuse-archivemount \
                            crash crash-access crash-user-access fuse-drivefs regmond_senders \
                            arc-camera camera pkcs11 policy-readers arc-keymasterd debugfs-access \
                            portage steam render hidraw cros-disks disk watchdog virtaccess \
                            iioservice i2c drm_dp_aux serial kmem cros_ec-access power bluetooth \
                            crosvm crosvm-root vm_cicerone seneschal seneschal-dbus pluginvm \
                            vtpm mtp cups sshd smdisplay session_manager; do
                        ...
                        /usr/sbin/getent group 604  >/dev/null    || groupadd -g 604 arc-sensor 2>/dev/null
                        ...
                        /usr/sbin/getent group 600  >/dev/null    || groupadd -g 600 cras 2>/dev/null
                        /usr/sbin/getent group 85   >/dev/null    || groupadd -g 85 usb 2>/dev/null
                        /usr/sbin/getent group 20162 >/dev/null   || groupadd -g 20162 traced-producer 2>/dev/null
                        /usr/sbin/getent group 20164 >/dev/null   || groupadd -g 20164 traced-consumer 2>/dev/null
                        /usr/sbin/getent group 1001 >/dev/null    || groupadd -g 1001 chronos-access 2>/dev/null
                        /usr/sbin/getent group 240  >/dev/null    || groupadd -g 240 brltty 2>/dev/null
                        /usr/sbin/getent group 20150 >/dev/null   || groupadd -g 20150 arcvm-boot-notification-server 2>/dev/null
                        /usr/sbin/getent group 20189 >/dev/null   || groupadd -g 20189 arc-mojo-proxy 2>/dev/null
                        /usr/sbin/getent group 20152 >/dev/null   || groupadd -g 20152 arc-host-clock 2>/dev/null
                        /usr/sbin/getent group 608  >/dev/null    || groupadd -g 608 midis 2>/dev/null
                        /usr/sbin/getent group 415  >/dev/null    || groupadd -g 415 suzy-q 2>/dev/null
                        /usr/sbin/getent group 612  >/dev/null    || groupadd -g 612 ml-core 2>/dev/null
                        /usr/sbin/getent group 311  >/dev/null    || groupadd -g 311 fuse-archivemount 2>/dev/null
                        /usr/sbin/getent group 20137 >/dev/null   || groupadd -g 20137 crash 2>/dev/null
                        /usr/sbin/getent group 419  >/dev/null    || groupadd -g 419 crash-access 2>/dev/null
                        /usr/sbin/getent group 420  >/dev/null    || groupadd -g 420 crash-user-access 2>/dev/null
                        /usr/sbin/getent group 304  >/dev/null    || groupadd -g 304 fuse-drivefs 2>/dev/null
                        /usr/sbin/getent group 20215 >/dev/null   || groupadd -g 20215 regmond_senders 2>/dev/null
                        /usr/sbin/getent group 603  >/dev/null    || groupadd -g 603 arc-camera 2>/dev/null
                        /usr/sbin/getent group 20042 >/dev/null   || groupadd -g 20042 camera 2>/dev/null
                        /usr/sbin/getent group 208  >/dev/null    || groupadd -g 208 pkcs11 2>/dev/null
...
CONFIG_VIRTIO_PCI=y
CONFIG_DRM_SCHED=y
CONFIG_DRM_SCHED_JOB=y
CONFIG_DRM_SCHED_ENTITY=y
CONFIG_DRM_SCHED_FENCE=y
EOF
        ;;
    aarch64)
        case "$GPU_VENDOR" in
