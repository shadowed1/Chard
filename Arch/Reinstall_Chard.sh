...
EOF
		...
		sudo tee "$CHARD_ROOT/bin/chard_flatpak_ns" >/dev/null <<'EOF'
#!/bin/bash
CHARD_HOME=$(cat /.chard_home)
CHARD_USER=$(cat /.chard_user)
export HOME=/$CHARD_HOME
...
EOF
		...
		sudo chmod +x "$CHARD_ROOT/bin/chard_flatpak_ns"
...
sudo chmod +x "$CHARD_ROOT/bin/chard_bazaar"
		...
		sudo tee "$CHARD_ROOT/bin/chard_xfce4" >/dev/null <<'EOF'
#!/bin/bash
XFCE4="$1"
if [ -z "$XFCE4" ]; then
    exit 1
...
sudo chmod +x "$CHARD_ROOT/bin/chard_xfce4"
...
sudo tee "$CHARD_ROOT/bin/chard_blender" >/dev/null <<'EOF'
#!/bin/bash
CHARD_HOME=$(cat /.chard_home)
CHARD_USER=$(cat /.chard_user)
export HOME=/$CHARD_HOME
