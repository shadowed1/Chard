...
task
script
    logger -t chard "[chard_baguette]: starting"
    BASHRC=""
    attempt=0
    max_attempts=10
    wait_seconds=60
    ...
    max_attempts=10
    while [ "$attempt" -lt "$max_attempts" ]; do
        logger -t chard "[chard_baguette]: starting termina (attempt $((attempt+1))/$max_attempts)"
        if vmc start --enable-gpu --vm-type BAGUETTE --no-shell termina; then
            logger -t chard "[chard_baguette]: termina started successfully"
            exit 0
        fi
        attempt=$((attempt + 1))
