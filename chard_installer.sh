...
echo ""
sudo mkdir -p /opt/chard
sudo mkdir -p /opt/chard/flatpak
sudo mkdir -p /opt/chard/flatpak-deps
...
download_and_extract "$URL" "/opt/chard/flatpak"

URL="https://archlinux.org/packages/extra/x86_64/ostree/download"
download_and_extract "$URL" "/opt/chard/flatpak-deps"
...
download_and_extract "$URL" "/opt/chard"
...
export LD_LIBRARY_PATH="/opt/chard/flatpak-deps/usr/lib:$LD_LIBRARY_PATH"
...
if file "$XDG_RUNTIME_DIR/dbus-session" | grep -q socket; then
  ...
  grep -v '^export DBUS_SESSION_BUS_ADDRESS=' "/opt/chard/.chardEnv" > "/opt/chard/chardEnv.tmp"
  echo "export DBUS_SESSION_BUS_ADDRESS=\"$DBUS_SESSION_BUS_ADDRESS\"" >> "/opt/chard/.chardEnv.tmp"
  mv "/opt/chard/.chardEnv.tmp" "/opt/chard/.chardEnv"
else
...
FLATPAK_ENV_LINE='[ -f "/opt/chard/.chardEnv" ] && . "/opt/chard/.chardEnv"'
FLATPAK_LOGIC_LINE='[ -f "/opt/chard/.chardLogic" ] && . "/opt/chard/.chardLogic"'
...
echo ""
"/opt/chard/flatpak/usr/bin/flatpak" --version
sleep 3
...
export PATH="/opt/chard/usr/bin:$PATH"
CHARD_LIB="/opt/chard/usr/lib"
FLATPAK_USER_DIR="/opt/chard/share/flatpak" \
LD_LIBRARY_PATH="$CHARD_LIB:$LD_LIBRARY_PATH" \
"/opt/chard/flatpak/usr/bin/flatpak" --version
export XDG_DATA_DIRS="/opt/chard/usr/share:$XDG_DATA_DIRS"
export FLATPAK_USER_DIR="/opt/chard/share/flatpak"
mkdir -p "$XDG_RUNTIME_DIR"
chmod 700 "$XDG_RUNTIME_DIR"
NPM_BASE="/opt/chard/usr/lib/node_modules/npm"
NVM_DIR="/opt/chard/usr/share/nvm"
BIN_DIR="/opt/chard/usr/bin"
...
ln -sf "/opt/chard/bin/chard" "/opt/chard/bin/yay"
ln -sf "/opt/chard/bin/chard" "/opt/chard/bin/paru"
ln -sf "/opt/chard/bin/chard" "/opt/chard/bin/pacaur"
ln -sf "/opt/chard/bin/chard" "/opt/chard/bin/pacman"
