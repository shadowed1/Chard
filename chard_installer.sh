...
echo ""
mkdir -p /usr/local/bin/chard
mkdir -p /usr/local/bin/chard/flatpak
mkdir -p /usr/local/bin/chard/flatpak-deps
...
download_and_extract "$URL" "/usr/local/bin/chard/flatpak"

URL="https://archlinux.org/packages/extra/x86_64/ostree/download"
download_and_extract "$URL" "/usr/local/bin/chard/flatpak-deps"
...
download_and_extract "$URL" "/usr/local/bin/chard"
...
echo "${RESET}${GREEN}"
curl -L https://raw.githubusercontent.com/shadowed1/Chard/main/.chardLogic -o /usr/local/bin/chard/.chardLogic
echo "${RESET}${YELLOW}"
curl -L https://raw.githubusercontent.com/shadowed1/Chard/main/.chardEnv -o /usr/local/bin/chard/bin/.chardEnv
echo "${RESET}${GREEN}"
curl -L https://raw.githubusercontent.com/shadowed1/Chard/main/chard -o /usr/local/bin/chard/bin/chard
echo "${RESET}${YELLOW}"
curl -L https://raw.githubusercontent.com/shadowed1/Chard/main/version -o /usr/local/bin/chard/bin/version
echo "${RESET}"
chmod +x /usr/local/bin/bin/chard
echo ""

export LD_LIBRARY_PATH="/usr/local/bin/chard/flatpak-deps/usr/lib:$LD_LIBRARY_PATH"
...
if file "$XDG_RUNTIME_DIR/dbus-session" | grep -q socket; then
  ...
  grep -v '^export DBUS_SESSION_BUS_ADDRESS=' "/usr/local/bin/chard/.chardEnv" > "/usr/local/bin/chard/chardEnv.tmp"
  echo "export DBUS_SESSION_BUS_ADDRESS=\"$DBUS_SESSION_BUS_ADDRESS\"" >> "/usr/local/bin/chard/.chardEnv.tmp"
  mv "/usr/local/bin/chard/.chardEnv.tmp" "/usr/local/bin/chard/.chardEnv"
else
...
FLATPAK_ENV_LINE='[ -f "/usr/local/bin/chard/.chardEnv" ] && . "/usr/local/bin/chard/.chardEnv"'
FLATPAK_LOGIC_LINE='[ -f "/usr/local/bin/chard/.chardLogic" ] && . "/usr/local/bin/chard/.chardLogic"'
...
echo ""
"/usr/local/bin/chard/flatpak/usr/bin/flatpak" --version
sleep 3
...
export PATH="/usr/local/bin/chard/usr/bin:$PATH"
CHARD_LIB="/usr/local/bin/chard/usr/lib"
FLATPAK_USER_DIR="/usr/local/bin/chard/share/flatpak" \
LD_LIBRARY_PATH="$CHARD_LIB:$LD_LIBRARY_PATH" \
"/usr/local/bin/chard/flatpak/usr/bin/flatpak" --version
export XDG_DATA_DIRS="/usr/local/bin/chard/usr/share:$XDG_DATA_DIRS"
export FLATPAK_USER_DIR="/usr/local/bin/chard/share/flatpak"
mkdir -p "$XDG_RUNTIME_DIR"
chmod 700 "$XDG_RUNTIME_DIR"
NPM_BASE="/usr/local/bin/chard/usr/lib/node_modules/npm"
NVM_DIR="/usr/local/bin/chard/usr/share/nvm"
BIN_DIR="/usr/local/bin/chard/usr/bin"

mkdir -p "$BIN_DIR"
mkdir -p /usr/local/bin/etc/xdg/xfce4/xfwm4
...
ln -sf "/usr/local/bin/chard/bin/chard" "/usr/local/bin/chard/bin/yay"
ln -sf "/usr/local/bin/chard/bin/chard" "/usr/local/bin/chard/bin/paru"
ln -sf "/usr/local/bin/chard/bin/chard" "/usr/local/bin/chard/bin/pacaur"
ln -sf "/usr/local/bin/chard/bin/chard" "/usr/local/bin/chard/bin/pacman"
