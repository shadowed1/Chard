...
if [[ -n "$gcc_version" && -n "$CHOST" ]]; then
    ...
    if [[ -d "$gcc_bin_path" ]]; then
        export PATH="$PYEXEC_DIR:$PATH:$ROOT/usr/bin:/usr/lib/ccache/bin:$ROOT/bin:$gcc_bin_path"
    else
        export PATH="$PYEXEC_DIR:$PATH:$ROOT/usr/bin:/usr/lib/ccache/bin:$ROOT/bin:$ROOT/usr/$CHOST/gcc-bin/14"
    fi
