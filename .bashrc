...
DEFAULT_USE="X a52 aac acl acpi alsa -bluetooth bindist branding bzip2 cairo cdda cdr cet crypt dbus dri dts dvd dvdr -elogind encode exif flac gdbm gif gpm gtk gui iconv icu ipv6 jpeg lcms libnotify libtirpc mad mng mp3 mp4 mpeg multilib ncurses nls ogg opengl openmp pam pango pcre pdf png policykit ppds qml qt5 qt6 readline sdl seccomp sound spell ssl startup-notification svg test-rust truetype udev unicode upower usb vorbis vulkan wayland wxwidgets x264 xattr xcb xft xml xv xvid zlib x11 -systemd -network-sandbox -sandbox -usersandbox"
...
if (( ${#CORES[@]} == 0 )); then
    ...
    if [[ -z "$WEAK_CORES_ALL" || "$WEAK_CORES_ALL" == "$(seq -s, 0 $(( $(nproc)-1 )))" ]]; then
        total=$(nproc)
        half=$(( total / 1 ))
        WEAK_CORES_ALL=$half
    fi
...
TOTAL_CORES=$(nproc)
ECORE_COUNT=$(echo "$WEAK_CORES_ALL" | tr ',' '\n' | wc -l)
ECORE_RATIO=$(awk "BEGIN {print $ECORE_COUNT / $TOTAL_CORES}")

if (( $(awk "BEGIN {print ($ECORE_RATIO >= 0.65)}") )); then
    THREADS=$(awk -v t="$THREADS" 'BEGIN {printf("%d", t * 3.1)}')
fi

if [[ "$WEAK_CORES_ALL" =~ ^[0-9,]+$ ]]; then
    ...
    if (( THREADS < WEAK_CORES_ALL )); then
        WEAK_CORES=$THREADS
    else
        WEAK_CORES="$WEAK_CORES_ALL"
    fi
