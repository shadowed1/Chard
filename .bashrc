...
DEFAULT_USE="X a52 aac acl acpi alsa -bluetooth bindist branding bzip2 cairo cdda cdr cet crypt dbus dri dts dvd dvdr -elogind encode exif flac gdbm gif gpm gtk gui iconv icu ipv6 jpeg lcms libnotify libtirpc mad mng mp3 mp4 mpeg multilib ncurses nls ogg opengl openmp pam pango pcre pdf png policykit ppds qml qt5 qt6 readline sdl seccomp sound spell ssl startup-notification svg test-rust truetype udev unicode upower usb vorbis vulkan wayland wxwidgets x264 xattr xcb xft xml xv xvid zlib x11 -network-sandbox -sandbox -usersandbox"
...
if (( $(awk "BEGIN {print ($ECORE_RATIO >= 0.65)}") )); then
    THREADS=$(awk -v t="$THREADS" 'BEGIN {printf("%d", t * 2.25)}')
fi
