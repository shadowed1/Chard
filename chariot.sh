...
ARCH=$(uname -m)
if [ "$ARCH" != "x86_64" ]; then
    echo "Architecture is $ARCH, skipping Steam setup (requires x86_64)."
    exit 0
fi

echo "Architecture is x86_64, proceeding with Steam setup..."

sudo -E eselect repository enable steam-overlay
sudo -E emaint sync -r steam-overlay
sudo -E emerge --sync
sudo -E emerge --oneshot sys-apps/portage
...
rm -rf /var/tmp/portage/sys-libs/ncurses-*
rm -rf /var/tmp/portage/games-util/steam-launcher-*
eclean-dist -d

}
run_checkpoint 129 "sudo -E emerge games-util/steam-launcher" checkpoint_129
