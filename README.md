...
### Supports ChromeOS, Flex, mainstream Linux distros, Asahi Linux for Apple Silicon, and more!
__ChromeOS requires developer mode. crew-sudo should not be used for Chard; Use SudoCrosh instead__ 

*Enabling Developer Mode for ChromeOS:* <br>
https://www.chromium.org/chromium-os/developer-library/guides/device/developer-mode/#enable-developer-mode
...
- `chard_repair` -- Repair Chard markers in case of issues. 
...
- 0.49: `Fixed several typos on Gentoo's Chariot by yesidodo73, iamdays confirming selinux package install was useless and removed, NotRX confirming selinux bug, spotting several Arch Chariot useless packages, bug reports and Steam suggestion. Fixed language setup. Fixed chard run issue for Chard Arch. Added Dynamic LD Preload scan. Fixed Chard Gentoo's XDG_DATA_DIRS. Added full $CHARD_ROOT path for commands on reinstall. chard_icons folder renamed to .chard_icons`
...
- *days* - Helping fix audio issues with Wine, chard_garcon, enabled mouse capture/lock, isolating Steam Remote Play issues, GPU Screen Recorder, reporting bugs, creating proot syntax that I struggle with, and more. 
